...
import numpy as np
from data import get_vocab, load_train_data_joint, data_iter_joint_combine
from model_joint_combine import JOINT_COMBINE
...
for i in range(10):
    print('file ' + str(i+1))
    fw_ner = open(dirs + files_t[i] + '.joint.combine.ner2', 'w')
    fw_sa = open(dirs + files_t[i] + '.joint.combine.sa2', 'w')
    print('load train')
    doc, ner_, sa_, sparse_, _, _ = load_train_data_joint(dirs+files[i], vocab_w2i, sen_len, sparse_len, crf_num, ner_num, sa_num, ner_onehot, sa_onehot)
    print('load test')
    doc_t, ner_t, sa_t, sparse_t, _, _ = load_train_data_joint(dirs+files_t[i], vocab_w2i, sen_len, sparse_len, crf_num, ner_num, sa_num, ner_onehot, sa_onehot)
    with tf.Graph().as_default():
        sess = tf.Session()
        with sess.as_default():
            lstm = JOINT_COMBINE(batch_size, sen_len, num_hidden, ner_num, sa_num, vocab_size, embedding_size, embedding, learning_rate, sparse_len, crf_num)
            sess.run(tf.initialize_all_variables())
            def train_step(input_, ner_tr, sa_tr, sparse_tr, d):
                feed_dict = {
                    lstm.input : input_,
                    lstm.sparse: sparse_tr,
                    lstm.ner : ner_tr,
                    lstm.sa  : sa_tr,
		    lstm.dropout: d
                }
                _, lss = sess.run([lstm.trains, lstm.loss], feed_dict)
            def test_step(input_, ner_t_, sa_t_, sparse_t_, d, fw_ner, fw_sa, trans_ner, trans_sa):
                totals_ner_ = 0
                corrects_ner_ = 0
                totals_sa_ = 0
                corrects_sa_ = 0
                feed_dict = {lstm.input : input_, lstm.sparse : sparse_t_, lstm.dropout : d}
            ...
            data = data_iter_joint_combine(doc, ner_, sa_, sparse_, num_epoch, batch_size)
            for input_, ner_tr, sa_tr, sparse_tr in data:
                train_step(input_, ner_tr, sa_tr, sparse_tr, True)
            corrects_ner = 0
            totals_ner = 0
            corrects_sa = 0
            totals_sa = 0
            test_data = data_iter_joint_combine(doc_t, ner_t, sa_t, sparse_t, 1, 1)
	    trans_ner = sess.run(lstm.trans_ner)
            trans_sa = sess.run(lstm.trans_sa)
            for input_, ner_t_, sa_t_, sparse_t_, in test_data:
                corrects_ner_, totals_ner_, corrects_sa_, totals_sa_ = test_step(input_, ner_t_, sa_t_, sparse_t_, False, fw_ner, fw_sa, trans_ner, trans_sa)
