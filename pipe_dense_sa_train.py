...
import numpy as np
from data import get_vocab, load_train_data_pipe, data_iter
...
for i in range(10):
    print('file ' + str(i+1))
    fw = open(dirs + files_t[i] + '.pipe.dense1', 'w')
    print('load train')
    doc, label, sparse, _ = load_train_data_pipe(dirs+files[i], vocab_w2i, sen_len, sparse_len, crf_num, label_num, label_onehot)
    print('load test')
    doc_t, label_t, sparse_t, _ = load_train_data_pipe(dirs+files_t[i], vocab_w2i, sen_len, sparse_len, crf_num, label_num, label_onehot)
    with tf.Graph().as_default():
        sess = tf.Session()
        with sess.as_default():
            lstm = LSTM(sen_len, num_hidden, label_num, vocab_size, embedding_size, embedding, learning_rate)
            ...
            def test_step(input_, label_, d, fw, trans):
                totals_ = 0
                corrects_ = 0
	    ...
	    trans = sess.run(lstm.trans)
            for input_, label_ in test_data:
                corrects_, totals_ = test_step(input_, label_, False, fw, trans)
                corrects += corrects_
                totals += totals_
